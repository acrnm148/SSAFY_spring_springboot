...
%>
    ...
    <script>
    	...
    	window.onload = function(){
    		...
    		// 파일 업로드 checkbox 
    		document.querySelector("#chkFileUploadInsert").addEventListener("change",(e)=>{
    			if(e.target.checked){
    		...
    		document.querySelector("#chkFileUploadUpdate").addEventListener("change",(e)=>{
    			if(e.target.checked){
    		...
    		// thumbnail image 처리 
    		document.querySelector("#inputFileUploadInsert").addEventListener("change",(e)=>{
    			const fileArray = Array.from(e.target.files); // 유사배열을 js의 객체 배열로 변환 
    			fileArray.forEach(file=>{
    				let reader = new FileReader();
    				reader.readAsDataURL(file); // 파일을 읽어 들인다. 
    		...
    		document.querySelector("#inputFileUploadUpdate").addEventListener("change",(e)=>{
    			const fileArray = Array.from(e.target.files); // 유사배열을 js의 객체 배열로 변환 
    			fileArray.forEach(file=>{
    				let reader = new FileReader();
    				reader.readAsDataURL(file); // 파일을 읽어 들인다. 
    	...
    	async function boardList(){
    		...
    		try{
    			...
    			const {result,count,list} = await response.json();
    			
    			if(result == SUCCESS){
    				makeListHtml(list);
    				// 총건수를 이미 가져왔으므로 바로 처리 
    				TOTAL_LIST_ITEM_COUNT = count;
    	...
    	async function boardInsert(){
    		// 기존의 x-www.. 파라미터 처리 대신 multipart/formdata
